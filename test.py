...
def single_scale_test(model, test_loader, test_list, save_dir):
    ...
    for idx, image in enumerate(test_loader):
        ...
        fuse_res = results[-1].squeeze(0).squeeze(0).numpy()
...
def multi_scale_test(model, test_loader, test_list, save_dir):
    ...
    for idx, image in enumerate(test_loader):
        ...
        for k in range(len(scale)):
            ...
            results = model(jt.unsqueeze(jt.float32(im_), 0))
            fuse_res = results[-1].squeeze(0).squeeze(0).numpy()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Jittor Testing')
    ...
    test_loader   = test_dataset.set_attrs(batch_size=1, num_workers=1, drop_last=False, shuffle=False)
