...
class RCF(nn.Module):
        ...
        if pretrained is not None:
            ...
            for k in vgg16.keys():
                ...
                if size == 2:
                    ...
                    params[name_space] = jt.array(data)
            self.load_state_dict(params)

    def _init_weights(self, m):
        if isinstance(m, nn.Conv2d):
            jt.init.gauss_(m.weight, 0, 0.01)
            if m.weight.shape == [1, 5, 1, 1]:
                jt.init.constant_(m.weight, 0.2)
    ...
    def _make_bilinear_weights(self, size, num_channels):
        ...
        filt = jt.array(filt)
        w = jt.zeros((num_channels, num_channels, size, size))
    ...
    def execute(self, x):
        ...
        out2 = nn.conv_transpose2d(out2, self.weight_deconv2, stride=2)
        out3 = nn.conv_transpose2d(out3, self.weight_deconv3, stride=4)
        out4 = nn.conv_transpose2d(out4, self.weight_deconv4, stride=8)
        out5 = nn.conv_transpose2d(out5, self.weight_deconv5, stride=8)
        ...
        fuse = concat((out1, out2, out3, out4, out5), dim=1)
        fuse = self.score_fuse(fuse)
        results = [out1, out2, out3, out4, out5, fuse]
        results = [jt.sigmoid(r) for r in results]
        return results
